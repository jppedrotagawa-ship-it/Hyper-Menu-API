...
-- banner de imagem no topo (sprite); fallback em texto
local function drawBanner(x0, y0, w, h)
    if Assets.ok and Assets.banner then
        DrawSprite(Assets.txd, 'banner', x0 + w / 2, y0 + h / 2, w, h, 0.0, 255, 255, 255, 255)
    else
        srect(x0, y0, w, h, 9, 12, 22, 250)
        text(x0 + 14, y0 + 30, 'HYPER MENU', 0.45, A.r, A.g, A.b)
        text(x0 + w - 12, y0 + 30, 'v' .. (C.localVersion or ''), 0.26, 150, 170, 200, true, { x0, x0 + w - 12 })
    end
end

-- barra de titulo (pagina atual) + dots de pagina
local function drawSub(x0, y0, w, title, pages, cur)
    srect(x0, y0, w, SUB_H, A.r, A.g, A.b, 80)
    text(x0 + 14, y0 + 9, title:upper(), 0.33, 255, 255, 255)
    local d = 6
    for i = 1, pages do
        local x = x0 + w - 14 - (pages - i) * (d + 6)
        rect(x, y0 + 15, d, d, A.r, A.g, A.b, cur == i and 255 or 80)
    end
end

local function drawItems(x0, y0, items, sc, cu, iw)
    local wrap = { x0 + 52, x0 + iw - 10 }
    ...
    for i = sc + 1, sc + maxRows do
        ...
        local sel = (i - 1) == cu
        if sel then
            glow(x0 + PAD, y + 1, iw - PAD * 2, ITEM_H - 2, A.r, A.g, A.b, 170)
            srect(x0 + PAD, y + 1, iw - PAD * 2, ITEM_H - 2, A.r, A.g, A.b, 95)
        end
        local lx = x0 + PAD + 6
        if it.cfg.icon then
            icon(x0 + PAD, y + 4, 26, it.cfg.icon)
            lx = x0 + PAD + 34
        end
        ...
        if it.kind == 'toggle' then
            local on = it.state
            text(x0 + iw - 10, y + 8, on and '[ON]' or '[OFF]', 0.29,
        ...
        elseif it.kind == 'cycle' then
            ...
            text(x0 + iw - 10, y + 8, '< ' .. (cur and cur[1] or '') .. ' >', 0.28,
                 120, 180, 255, true, wrap)
        elseif it.kind == 'list' then
            text(x0 + iw - 10, y + 8, '>>', 0.29, 120, 180, 255, true, wrap)
        elseif it.kind == 'action' then
            text(x0 + iw - 10, y + 8, '>>', 0.29, 205, 215, 230, true, wrap)
        end
    end
    if sc + maxRows < #items then
        local y = y0 + maxRows * ITEM_H - 14
        text(x0 + iw - 10, y, 'v', 0.30, A.r, A.g, A.b, true, wrap)
    end
end
...
local function drawUI()
    ...
    local x0 = math.floor(resW / 2 - MENU_W / 2)
    local y0 = math.floor(resH * 0.14)
    ...
    local maxRows = math.min(C.maxVisible, #items)
    local bodyH = maxRows * ITEM_H
    local H = BANNER_H + SUB_H + bodyH + FOOT_H

    -- fundo translucido (jogo aparece atras)
    rect(x0 - 3, y0 - 3, MENU_W + 6, H + 6, 0, 0, 0, 120)
    srect(x0, y0, MENU_W, H, 4, 6, 12, 200)

    drawBanner(x0, y0, MENU_W, BANNER_H)

    if modal then
        drawSub(x0, y0 + BANNER_H, MENU_W, title, 1, 1)
    else
        drawSub(x0, y0 + BANNER_H, MENU_W, title, #Menu.categories, Menu.catIdx)
    end

    drawItems(x0, y0 + BANNER_H + SUB_H, items,
              modal and modal.scroll or Menu.scroll,
              modal and modal.cursor or Menu.cursor, MENU_W)

    -- rodape
    srect(x0, y0 + H - FOOT_H, MENU_W, FOOT_H, 3, 5, 10, 200)
    text(x0 + 14, y0 + H - FOOT_H + 6, 'SETAS: trocar pagina/item - ENTER: ativar - BACKSPACE: fechar', 0.25, 140, 155, 175)
end
